	jQuery(function($){
	  ...
	  function buttonPress(e) {
		var ButtonID = (window.event) ? event.keyCode : e.keyCode;
		
		if(ButtonID == 37) moveLeftRight(true);
		if(ButtonID == 38) moveTopBottom(true);
		if(ButtonID == 39) moveLeftRight(false);
		if(ButtonID == 40) moveTopBottom(false);
		if(ButtonID == 107) zoomIn();
		if(ButtonID == 109) zoomOut();
	  } 	
	  function refreshSVG() {
		$('.loader').css('display','');
		createSVG();
